# Nigeria Macro-Economic Stability Analysis (2002–2025)

This repository contains the quantitative models and data visualizations supporting the research paper: **"The Merchant Bank Paradox: An Econometric Assessment of Structural Breaks in Monetary Transmission and Labor-Induced Institutional Decay in Nigeria (2002–2025):"**

## **Key Research Metrics**
* **Merchant Bank Paradox:** Modeled institutional correlation ($r = 0.9977$) during structural volatility clusters.
* **Liquidity Decay:** Quantified erosion of purchasing power with a calculated Beta coefficient ($\beta = -232,335.35$).
* **Labor Precariousness Index:** Analysis of $73.51\%$ non-permanent staffing risk in the financial sector.
* **A core finding of this research is the LPI, which demonstrates that institutional resilience is compromised by human capital fragility.The $73.51\%$ Threshold: Our data reveals that high reliance on contract/casual staff prevents banks from maintaining the "Institutional Memory" required to navigate high-volatility regimes.Transmission Failure: In the "Paradox Zone," the high turnover of professional staff creates a Short-Termism Bias, rendering standard Central Bank interest rate adjustments less effective as internal risk-management capacity decays.Finalize analysis: Integrated LPI 73.51% and Merchant Bank Paradox Correlation models.
* **2025 Dashboard:** Predictive analysis of the $34.6\%$ inflation peak and $63.1\%$ debt-service ratio.

## **Methodology**
The analysis utilizes **Time-Series Regression**, **Pearson Correlation**, and the **Chow Test for Structural Breaks**. It bridges the 2016 liquidity crisis with the 2023–2025 fiscal shocks, identifying the **"Paradox Zone"** where traditional monetary transmission mechanisms fail.

## **Core Research Evidence (Visuals)**
*Click the links below to view the high-resolution econometric dashboards:*

*  **[Figure 1: Long-Term Inflation Trend (2003-2025)](inflation_trend_2003_2025.png)**
*  **[Figure 2: Systemic Risk & Volatility Clusters](systemic_risk_volatility.png)**
*  **[Figure 3: Liquidity Decay Index](liquidity_decay.png)**
*  **[Figure 4: The Merchant Bank Paradox Nexus](merchant_bank_paradox_final.png)**
*  **[Figure 5: The Labor Precariousness Index](labor_precariousness_index.png)**
## **Repository Structure**
* **cleaned_inflation_data.csv**: Cleaned time-series dataset.
* **Nigeria_Macro_Analysis.ipynb**: Updated Jupyter Notebook with Python code and Chow Test results.
* **requirements.txt**: Environment setup for computational reproducibility.
* **LICENSE**: MIT Open Source License.

---
**Author:** Adedeji Bamidele, M.Sc.  
**Manuscript ID:** WDP-D-26-00263  
**ORCID / Submission ID:** 265496966
